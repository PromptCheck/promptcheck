...
# Install promptcheck from TestPyPI and its dependencies
...
# Configure Poetry to use the system Python where promptcheck is now installed
...
# The `promptcheck` command installed by pip will be on the PATH.

ENTRYPOINT ["/app/entrypoint.sh"]

# The command `promptcheck` should be available on the PATH after `poetry install` 
...
# Then `promptcheck` command will be available.

# Default command can be overridden in action.yml
# ENTRYPOINT ["poetry", "run", "promptcheck"]
# CMD ["--help"]

# For the action, the ENTRYPOINT will likely be a script that handles inputs
# or action.yml will directly call `poetry run promptcheck ...` 