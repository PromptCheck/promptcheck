...
import nltk # For BLEUMetric later and tokenization if needed
...
if __name__ == '__main__':
    ...
    mock_test_case_pass = TestCase(
        name="TestExactPass",
        input={"prompt": "Hi"},
        expected={"exact_match_string": "Hello there"},
        metrics=[sample_mc_config]
    )
    ...
    print(f"Exact Match (Pass) Result: {result_pass.json(indent=2)}")
    assert result_pass.passed is True

    mock_test_case_fail = TestCase(
        name="TestExactFail",
        input={"prompt": "Hi"},
        expected={"exact_match_string": "General Kenobi"},
        metrics=[sample_mc_config]
    )
    ...
    print(f"Exact Match (Fail) Result: {result_fail.json(indent=2)}")
    ...
    print(f"Exact Match (LLM Error) Result: {result_llm_error.json(indent=2)}")
    ...
    mock_test_case_no_expected = TestCase(
        ...
        input={"prompt": "Hi"},
        expected={}, # No exact_match_string
        metrics=[sample_mc_config]
    )
    result_no_expected = exact_match.calculate(mock_test_case_no_expected, mock_llm_response_pass)
    print(f"Exact Match (No Expected String) Result: {result_no_expected.json(indent=2)}")
